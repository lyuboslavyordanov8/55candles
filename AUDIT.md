...
| **B-03** | Commerce core   | **No product has a price.** `price` is optional on the type and undefined on all six products. Currency is therefore also undecided — see B-12.                                                                                                                                                                                                                                                                                                       | `src/types/product.ts:42-45`; `src/data/products.ts:3-218`                                                                                                                                                                                                | Cannot sell                                                               | **M**                   |
| **B-04** | Commerce core   | **Product model is missing every commerce field:** no `id`, `sku`, `stock`/inventory, `weight` or dimensions (required for courier pricing), `vatRate`, variants/size options, or `active` flag. Only one image per product plus an optional hover image.                                                                                                                                                                                             | `src/types/product.ts:1-49`                                                                                                                                                                                                                               | Cannot price shipping, cannot prevent overselling                         | **M**                   |
| **B-05** | Commerce core   | **No cart.** No cart state, no storage (cookie/localStorage/server), no guest-vs-logged-in strategy, no line items.                                                                                                                                                                                                                                                                                                                                   | verified — no cart module in `src/`                                                                                                                                                                                                                       | Cannot transact                                                           | **M**                   |
| **B-06** | Commerce core   | **Add to cart is a permanently disabled button** with the label baked into both locale files as "coming soon".                                                                                                                                                                                                                                                                                                                                        | `src/app/[locale]/products/[slug]/page.tsx:101-106`; `messages/en.json:62`; `messages/bg.json:62`                                                                                                                                                         | Shop is visibly non-functional                                            | **S** (once B-05 lands) |
| **B-07** | Commerce core   | **No checkout flow.** No address capture, no shipping method selection, no order summary, no confirmation page.                                                                                                                                                                                                                                                                                                                                       | verified                                                                                                                                                                                                                                                  | Cannot transact                                                           | **L**                   |
...
| **S-02** | Quality       | **A data test asserts a field that cannot exist.** `expect(p.price).toBeNull()` fails because `price` is `undefined`, never `null`. The test encodes "products have no price" as intended behaviour — delete or invert it when B-03 lands.                                                                                                                                                                                                                                                                                                                                                                            | `src/data/__tests__/products.test.ts:16`                                                                                                    | Misleading test; blocks green CI                                       | **S**  |
...
| **S-12** | SEO           | **No structured data.** No `Product`, `Offer`, `Organization` or `BreadcrumbList` JSON-LD. `Offer` requires a price, so this is gated on B-03.                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | verified                                                                                                                                    | No rich results                                                        | **S**  |
...
| **N-12** | Ops           | No `.env.example` to document required configuration once B-01 lands.                                                                                                                                                           | verified                                                                                        | Onboarding                                         | S                          |
