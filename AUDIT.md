...
| Q-25 delivery methods | Done, unpriced | `src/lib/shipping.ts` — 2 couriers × 3 methods, weight-banded tariffs, all rate cards empty (Q-22) |
...
**What is still missing, and from whom:** price and packed weight per product (Q-11/Q-12, owner); Econt and Speedy rate
cards and API credentials (Q-22, merchant contracts); Stripe account and keys (Q-20); free-delivery threshold (Q-24);
who bears the COD fee (Q-23 — currently modelled as the merchant, the safer default); and a database, without which
checkout validates and prices an order but cannot store one (Q-34). The checkout page is `noindex` until it can.
...
| **B-03** | Commerce core   | **MACHINERY DONE (Phase 1a) — the pricing table is still empty; see "Phase 1a status" above.** ~~No product has a price.~~ Prices now live in `src/data/pricing.ts` keyed by slug, in integer EUR cents, and the table has all six slugs commented out awaiting real numbers (Q-11/Q-12). Unpriced products render "price on request", never a blank or a zero.                                                                                                                                                                                                                                                                                                       | `src/types/product.ts:42-45`; `src/data/products.ts:3-218`                                                                                                                                                                                                | Cannot sell                                                               | **M**                   |
