...
| Q-23 who is billed, on the waybill | **Settled: the shop bears both, the customer hands over exactly the total** | `PAYMENT_SIDE` in `src/lib/couriers/econt.ts`. The gap measured on 2026-09-20 was real: the label we built then billed *both* the carriage and the наложен платеж fee to the RECEIVER, so the customer would have paid 3.74 EUR more at the counter than the site said — an undisclosed charge at delivery, and the fee is **3.24% of the collected sum, uncapped** (0.65 EUR on a 19.99 order, 3.24 on a 100), not the flat 0.60 the stand-in tariff card assumes. Nobody was ever affected: no waybill existed to be billed. Fixed with label creation as planned, but **not** with the `paymentReceiverAmount` split sketched here — that still adds the courier’s fees on top of `cdAmount`, and the delivery the customer paid for is already inside it. `paymentSenderMethod: 'cash'` on both the quote and the booking instead: measured 2026-09-21, the same parcel totals 3.74 either way and only `senderDueAmount`/`receiverDueAmount` move, so the price the checkout shows is unchanged and `COD_FEE_PAID_BY = 'merchant'` is now true of the parcel as well as the page. What it costs us is real and uncapped — see the tariff row below |
